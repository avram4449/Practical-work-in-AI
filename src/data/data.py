...
from .tox21_dataset import Tox21Dataset
...
class TorchVisionDM(BaseDataModule):
    def __init__(
        ...
        if self.dataset == "mnist":
            ...
            base_train = Tox21Dataset(
                csv_path="C:/FILES/jku/Semester 6/Practical work/realistic-al-open-source/src/data/tox21_compoundData_wSmiles.csv",
                split="train",
                max_train_samples=None  # Use all available train samples
            )
            # Wrap with ActiveLearningDataset for AL support
            self.train_dataset = ActiveLearningDataset(base_train)
            self.train_dataset.n_classes = 12  
            self.val_dataset = Tox21Dataset(
                csv_path="C:/FILES/jku/Semester 6/Practical work/realistic-al-open-source/src/data/tox21_compoundData_wSmiles.csv",
                split="val"
            )
            self.test_dataset = Tox21Dataset(
                csv_path="C:/FILES/jku/Semester 6/Practical work/realistic-al-open-source/src/data/tox21_compoundData_wSmiles.csv",
                split="test"
            )
            return
